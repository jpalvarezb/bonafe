...
export async function listPlannedActivities(
  ...
  const rows = await db
    ...
    .orderBy(asc(plannedActivities.plannedDate), asc(plannedActivities.createdAt));
...
export async function createPlannedActivity(
  ...
  if (input.parcelId) {
    farmId = await assertParcelInOrg(ctx, input.parcelId);
  }

  await assertActivityTypeInOrg(ctx, input.activityTypeId);

  if (input.cropCycleId) {
    await assertCropCycleInOrg(ctx, input.cropCycleId);
  }
...
export async function updatePlannedActivity(
  ...
  const [current] = await db
    ...
    .from(plannedActivities)
    .where(
      and(eq(plannedActivities.id, id), eq(plannedActivities.orgId, ctx.org.id)),
    )
    .limit(1);
  ...
  if (input.parcelId !== undefined) {
    farmId = input.parcelId
      ? await assertParcelInOrg(ctx, input.parcelId)
      : null;
  }
...
export async function convertPlannedActivity(ctx: OrgContext, id: string) {
  ...
  // The activity reuses the plan's own UUID: createActivity is idempotent by
  // id (ON CONFLICT DO NOTHING + replay fetch), so if the status update below
  // fails and the user retries, the same activity row is returned instead of
